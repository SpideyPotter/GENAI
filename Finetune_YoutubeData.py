"""
Continue fine-tuning on YouTube QA data.

Default behavior:
- Start from existing website-finetuned model:
  FineTunedmodels/Finetuned_WebsiteData/llama3_agri_finetuned/merged_16bit
- Train on:
  greenagriculture_QA/all_qa_pairs.jsonl
- Save new adapters/model to:
  FineTunedmodels/Finetuned_WebsiteandYoutubeData
"""
...
class Config:
    ...
    base_model: str = "FineTunedmodels/Finetuned_WebsiteData/llama3_agri_finetuned/merged_16bit"
    output_dir: str = "FineTunedmodels/Finetuned_WebsiteandYoutubeData"
    ...
    report_to: str = "none"
    run_name: str = "llama3-agri-website-youtube"
    lora_r: int = 16
    lora_alpha: int = 16
...
def load_model_and_tokenizer(cfg: Config):
    ...
    if cfg.gradient_checkpointing:
        ...
        # Required with gradient checkpointing on mostly-frozen PEFT models.
...
def train(cfg: Config):
    ...
    # Handle TRL API differences: processing_class vs tokenizer
    ...
    trainer = SFTTrainer(**{k: v for k, v in trainer_kwargs.items() if k in params or k in {"model", "args", "train_dataset", "eval_dataset", "data_collator"}})

    # Safety check: ensure at least one parameter is trainable before training.
    ...
    # Workaround for accelerate/optimizer mismatch that expects train()/eval().
    ...
    log.info("Starting training from %s", cfg.base_model)
    stats = trainer.train()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune website model on YouTube QA data")
