"""
Fine-tune Meta Llama 3.1 8B (base) with QLoRA on JSONL QA data.

Uses only `input` and `output` from each JSONL row by default (the `instruction`
field is ignored). Pass --include_instruction to prepend instruction as system.

Run (example):
  HF_TOKEN=... python finetune_stage1_llama31_8b_base.py \\
    --data_path finetune_QA_stage1/master_dataset.jsonl

Defaults target 1x H200 (~70GB VRAM), 16 CPU cores, ~30GB RAM. If the job is killed
with exit 137, lower --batch_size and/or --num_workers (host RAM pressure).
"""
...
class FinetuneConfig:
    ...
    output_dir: str = "./llama31-8b-base-agro-qlora"
...
def _format_llama31_messages_manual(messages: List[Dict[str, str]]) -> str:
    """Llama 3.1 turn format when tokenizer.chat_template is missing (base checkpoints)."""
    parts = ["<|begin_of_text|>"]
    for m in messages:
        role = m["role"]
        parts.append(
...
def prepare_datasets(cfg: FinetuneConfig, tokenizer) -> tuple[Dataset, Dataset]:
...
def _build_sft_config(cfg: FinetuneConfig) -> SFTConfig:
    ...
    # TRL >= 1.0 uses max_length on SFTConfig; older TRL used max_seq_length
...
def build_trainer(cfg: FinetuneConfig, model, tokenizer, train_ds, eval_ds):
    ...
    if "processing_class" in st_params:
        ...
        raise RuntimeError(
            ...
            "Check trl version against huggingface/trl docs."
        )
...
def main():
    parser = argparse.ArgumentParser(description="QLoRA fine-tune Llama 3.1 8B base on QA JSONL")
    parser.add_argument("--data_path", default="finetune_QA_stage1/master_dataset.jsonl")
    parser.add_argument("--output_dir", default="./llama31-8b-base-agro-qlora")
    parser.add_argument("--model_name", default="meta-llama/Meta-Llama-3.1-8B")
    parser.add_argument(
        "--include_instruction",
        action="store_true",
        help="Use JSONL 'instruction' as system message (default: ignore)",
    )
    ...
    if torch.cuda.is_available():
        ...
        logger.warning("No CUDA — training on CPU will be impractical.")
